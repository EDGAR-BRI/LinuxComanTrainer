...
export default function FileTree({ vfs }: FileTreeProps) {
  ...
  const renderTree = (node: any, prefix: string = '', isLast: boolean = true): JSX.Element => {
        ...
        {node.children && (
          <div className="ml-4">
            {node.children.map((child: any, i: number) => 
              renderTree(child, prefix + (isLast ? '    ' : '│   '), i === node.children.length - 1)
            )}
          </div>
        )}
      </div>
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 mb-4 text-sm">
      ...
      {/* Current directory contents - simple list */}
      <div className="space-y-1 mb-2">
        {files.map((f, i) => (
          <div key={i} className="flex items-center gap-2 text-gray-300 pl-2">
      ...
      {/* Optional full tree with hierarchy */}
      {showFullTree && (
        <div className="mt-3 pt-3 border-t border-gray-700 overflow-x-auto font-mono text-xs">
