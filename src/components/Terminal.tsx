...
export default function Terminal() {
  ...
  const [history, setHistory] = useState<HistoryEntry[]>([
    { input: '', output: 'Bienvenido al Linux Command Trainer', status: 'success' }
  ]);
      ...
      <div className="terminal-window">
        <div className="terminal-header">
          ...
          <div className="ml-auto flex items-center gap-4">
            <span className="text-gray-400 text-sm">{lessonIndex + 1}/{lessons.length}</span>
        ...
        <div className="p-6">
          {currentLesson && (
            <div className="bg-gray-800 rounded-lg p-6 mb-4">
              <h3 className="text-xl font-bold text-blue-400 mb-4">{currentLesson.title}</h3>
              <p className="text-gray-300 mb-4">{currentLesson.explanation}</p>
              <div className="bg-gray-900 rounded p-3 mb-4">
                <code className="text-green-400">Sintaxis: {currentLesson.syntax}</code>
              </div>
              <div className="mb-4">
                <h4 className="text-yellow-400 mb-2">Ejemplos:</h4>
                {currentLesson.examples.map((ex, i) => (
                  <div key={i} className="bg-gray-700 rounded p-2 mb-2">
                    <code className="text-green-300">{ex.cmd}</code>
          ...
          <div className="flex gap-4 justify-center">
            <button onClick={prevLesson} disabled={lessonIndex === 0} className="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded disabled:opacity-50">
              ← Anterior
            </button>
            <button onClick={nextLesson} disabled={lessonIndex === lessons.length - 1} className="bg-gray-700 hover:bg-gray-600 px-4 py-2 rounded disabled:opacity-50">
              Siguiente →
            </button>
          </div>
        </div>
  ...
  const renderMenu = () => (
    <div className="terminal-window w-full max-w-6xl mx-auto">
      ...
      <div className="p-6">
        <h2 className="text-3xl font-bold text-green-400 mb-8 text-center">Selecciona un Modo</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <button
            onClick={startLearnMode}
            className="p-8 rounded-lg border-2 border-blue-500 hover:bg-blue-900 hover:bg-opacity-30 transition-all hover:scale-105"
          >
            <div className="text-5xl mb-4">📚</div>
            <h3 className="text-2xl font-bold text-white mb-2">Modo Aprendizaje</h3>
            <p className="text-gray-400">Aprende comandos paso a paso con explicaciones y ejemplos</p>
            <div className="mt-4 text-sm text-gray-500">{lessons.length} lecciones disponibles</div>
          </button>

          <div
            onClick={() => {}}
            className="p-8 rounded-lg border-2 border-purple-500 hover:bg-purple-900 hover:bg-opacity-30 transition-all hover:scale-105 cursor-pointer"
          >
            <div className="text-5xl mb-4">🧪</div>
            <h3 className="text-2xl font-bold text-white mb-2">Modo Test</h3>
            <p className="text-gray-400">Pon a prueba tus conocimientos resolviendo desafíos</p>
        ...
        <div className="terminal-body h-48 overflow-y-auto">
          {history.map((entry, i) => (
            <div key={i} className="mb-2">
              {entry.input && (
                <div className="terminal-input-line">
